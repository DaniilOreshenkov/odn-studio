...
function setLang(lang) {
  ...
  if (r) r.textContent = curWords()[wi % curWords().length];
}
...
if (term) {
  const intro = [
    { t: 'prompt', txt: '➜ ' },
    { t: 'path', txt: '~/odn ' },
    { t: 'cmd', txt: './welcome --studio ODN', type: true },
    { t: 'br' },
  ...
  function appendLine(cls, text) {
    ...
    if (cls !== 'prompt' && cls !== 'path' && cls !== 'cmd') {
  ...
  function runIntro() {
    ...
    function step() {
      ...
      if (it.type) {
        ...
        function tc() {
          span.textContent = it.txt.slice(0, c++);

          if (c <= it.txt.length + 1) {
            setTimeout(tc, 36);
          } else {
            i++;
            setTimeout(step, 240);
        ...
        i++;
        setTimeout(step, it.t === 'ok' || it.t === 'out' ? 320 : 60);
      }
    }
  ...
  const cmds = {
    ...
    whoami() {
      return ['out', 'ODN — студия разработки цифровых продуктов 🚀'];
    },
  ...
  function showInput(skip) {
    ...
    inp.addEventListener('keydown', (e) => {
      if (e.key === 'Enter') {
        ...
        shown.className = 'cmd';
        shown.textContent = inp.value;

        inp.replaceWith(shown);
        term.appendChild(document.createElement('br'));
        ...
        if (val) {
          ...
          if (fn) {
            const r = fn.call(cmds);
            if (r) appendLine(r[0], r[1]);
          } else {
            appendLine('err', 'command not found: ' + val + '  →  напишите help');
          }
        }
...
function cycle() {
  ...
  setTimeout(() => {
    ...
    rot.classList.add('prep');

    requestAnimationFrame(() => {
      rot.classList.remove('prep');
...
if (window.matchMedia('(min-width:761px)').matches) {
  document.querySelectorAll('.btn-primary').forEach((b) => {
    b.addEventListener('mousemove', (e) => {
      ...
      b.style.transform = `translate(${x * 0.28}px,${y * 0.4}px) scale(1.04)`;
    });
  ...
  document.querySelectorAll('.port').forEach((p) => {
    p.addEventListener('mousemove', (e) => {
      ...
      p.style.transform = `perspective(900px) rotateY(${x * 6}deg) rotateX(${
        -y * 6
      }deg)`;
    });
